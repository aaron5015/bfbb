...
void iBackendResolve(void)
{
    ...
    // librw has one PLATFORM_D3D9 for both Direct3D backends and Vulkan, so
    // this is what tells it which of them to open. It has to be set before
    // Engine::open, which is where renderDevice() is first asked.
#ifdef RWD3D_MULTI
    rw::d3d::useD3D11 = (want == iSCREENBACKEND_D3D11);
    rw::d3d::useVulkan = (want == iSCREENBACKEND_VULKAN);
#endif
    ...
    case iSCREENBACKEND_VULKAN:
        // All three, because librw has one PLATFORM_D3D9: D3D11 and Vulkan read
        // and write the same native data and register the same pipelines.
        // Which device opens is the flags set above.
