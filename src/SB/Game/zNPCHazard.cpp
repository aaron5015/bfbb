...
// Buddy targeting is deliberately kept outside NPCHazard. NPCHazard is a
// reverse-engineered pooled object whose layout is shared by the rest of the
// game. Adding a member to it changes sizeof(NPCHazard), the stride of
// g_hazards, and every object offset after the insertion point. Keep the buddy
// association in a parallel table instead.
static const xVec3* g_buddy_targets[64] = { NULL };

static S32 BuddyHazardIndex(const NPCHazard* haz)
{
    if (haz < g_hazards || haz >= g_hazards + 64)
    {
        return -1;
    }
    return (S32)(haz - g_hazards);
}

void NPCHazard::SetBuddyTarget(const xVec3* target)
{
...
const xVec3* NPCHazard::GetBuddyTarget() const
{
    S32 idx = BuddyHazardIndex(this);
    return idx >= 0 ? g_buddy_targets[idx] : NULL;
}
...
S32 NPCHazard::ConfigHelper(en_npchaz haztype)
{
    ...
    case NPC_HAZ_FODBOMB:
    {
        ...
        // TEST 16: Upd_FodBomb() gates player damage on 0x2000, but the
        // reconstructed hazard flags did not set that bit.
        this->flg_hazard |= 0x2000;
    ...
    case NPC_HAZ_TUBELETBLAST:
    {
        ...
        this->flg_hazard |= 0x8b180;
        // TEST 16: Upd_TubeletBlast() gates player damage on 0x2000, but the
        // reconstructed hazard flags did not set that bit.
        this->flg_hazard |= 0x2000;
...
void NPCHazard::Reconfigure(en_npchaz haztype)
{
    ...
    const xVec3* buddy_old = this->GetBuddyTarget();
    xVec3 pos_old;
...
S32 NPCHazard::ColBuddySphere(F32 rad)
{
    ...
    if (!(this->flg_hazard & 0x40000000) && this->cb_notify != NULL)
    {
        this->cb_notify->Notify(HAZ_NOTE_HITBUDDY, this);
    }
    this->flg_hazard |= 0x40000000;
    return 1;
}

S32 NPCHazard::ColBuddyCyl(F32 rad, F32 hyt)
{
    ...
    if (xabs(delta.y) > hyt + 0.75f ||
        (delta.x * delta.x + delta.z * delta.z) > SQ(rad + 0.35f))
    {
        return 0;
    }
...
void NPCHazard::Upd_CattleProd(F32 dt)
{
    ...
    if ((this->flg_hazard & 0x2000) && this->tym_lifespan - this->tmr_remain > 0.25f)
    {
        S32 hit = 0;
        if (this->GetBuddyTarget() != NULL)
        {
            hit = this->ColBuddySphere(catprod->rad_cur);
        }
        else if (!(globals.player.DamageTimer > 0.0f))
        {
            hit = ColPlyrSphere(catprod->rad_cur);
        }
...
void NPCHazard::Upd_TTFlight(F32 dt)
{
    ...
    else
    {
        ...
        {
            S32 hit = 0;
            S32 buddy_hit = this->GetBuddyTarget() != NULL && ColBuddySphere(tartar->rad_cur);
            S32 player_hit = !(globals.player.DamageTimer > 0.0f) &&
            ...
            if (hit)
            {
                ...
                {
                    HurtThePlayer();
                    if (g_data_hazshrap[2] != NULL && g_data_hazshrap[2]->initCB != NULL)
                    {
...
void NPCHazard::Upd_TTSpill(F32 dt)
{
    ...
    {
        S32 hit = 0;
        S32 buddy_hit = this->GetBuddyTarget() != NULL && ColBuddySphere(ball->rad_cur);
        S32 player_hit = !(globals.player.DamageTimer > 0.0f) &&
                         ColPlyrSphere(ball->rad_cur);
        hit = buddy_hit || player_hit;
...
S32 NPCHazard::KickSteamyStinky()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (this->GetBuddyTarget() != NULL)
            {
                haz->SetBuddyTarget(this->GetBuddyTarget());
            }
...
void NPCHazard::Upd_TTStink(F32 dt)
{
    ...
    {
        S32 buddy_hit = this->GetBuddyTarget() != NULL && ColBuddySphere(ball->rad_cur);
        S32 player_hit = !(globals.player.DamageTimer > 0.0f) && ColPlyrSphere(ball->rad_cur);
        if (buddy_hit)
        {
            zBuddy_Damage(1);
        }
        if (player_hit)
        {
            ...
            if (g_data_hazshrap[2] != NULL && g_data_hazshrap[2]->initCB != NULL)
            {
                g_data_hazshrap[2]->initCB(g_data_hazshrap[2], globals.player.ent.model, NULL,
                                           NULL);
            }
        }
    }
...
void NPCHazard::Upd_OilBurst(F32 dt)
{
    ...
    if (this->flg_hazard & 0x2000)
    {
        ...
        if (this->GetBuddyTarget() != NULL)
        {
            hit = ColBuddySphere(0.75f * ball->rad_cur);
        }
        else if (!(globals.player.DamageTimer > 0.0f))
        {
            hit = ColPlyrSphere(0.75f * ball->rad_cur);
        }
...
void NPCHazard::Upd_MonCloud(F32 dt)
{
    ...
    if (this->GetBuddyTarget() != NULL && !zBuddy_IsAvailable())
    {
        this->SetBuddyTarget(NULL);
    }

    xEnt* plyr = &globals.player.ent;
    const xVec3* target_pos = this->GetBuddyTarget() != NULL ? this->GetBuddyTarget() : xEntGetPos(plyr);

    xVec3Sub(&dir_delta, target_pos, &this->pos_hazard);
    dir_delta.y += 4.0f;
    ...
    if (this->flg_hazard & 0x2000)
    {
        S32 buddy_hit = this->GetBuddyTarget() != NULL &&
                        this->ColBuddySphere(this->custdata.cloud.rad_cur);
        S32 player_hit = !(globals.player.DamageTimer > 0.0f) &&
    ...
    {
        _tagLightningAdd lyt;
        xVec3 lightning_target = *target_pos;

        NPCC_MakeLightningInfo(NPC_LYT_CLOUDZAP, &lyt);
        lyt.start = &this->pos_hazard;
        lyt.end = &lightning_target;
        lyt.time = 1.5f;
