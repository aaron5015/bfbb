...
// PC-only experiment: a Fodder variant that survives being hit/knocked around
// instead of dying to the first stun. It still dies instantly to anything
// that calls Damage() directly (goo/bad surfaces via CollideReview, hazards,
// scripted kills) since that path is untouched; only the "player bopped me"
// Stun() signal is changed to spend hitpoints instead of being lethal.
struct zNPCFodderTough : zNPCFodder
{
    zNPCFodderTough(S32 myType) : zNPCFodder(myType)
    {
    }
...
struct zNPCSlick : zNPCRobot
{
    ...
    // Hook called alongside the normal aimed shot during the attack goal.
    // Base Slick does nothing extra; forks (e.g. zNPCSlickBurst) override
    // this to add attack variety without touching the shared alert/attack
    // goal state machine. Returns the number of extra projectiles fired.
    virtual S32 FireRing(F32 spd_proj)
    {
        return 0;
    }
};

typedef struct zNPCSlickTouhou;

// Fork of zNPCSlick: same body, anims, and alert/attack goals, but its
// attack also flings a ring of oil bubbles out in a full circle around
// itself (in addition to the normal aimed shot at the player) via
// FireRing(). Register this type's bind name in g_tbltype in zNPCMgr.cpp
// to spawn it in place of a plain Slick.
struct zNPCSlickTouhou : zNPCSlick
{
    zNPCSlickTouhou(S32 myType) : zNPCSlick(myType)
    {
    }

    S32 FireRing(F32 spd_proj);
};
