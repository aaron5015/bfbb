    ...
    else if (npc->SomethingWonderful())
    {
        *trantype = GOAL_TRAN_SET;
        nextgoal = NPC_GOAL_IDLE;
                ...
                else if (!npc->arena.IncludesPlayer(0.0f, NULL) &&
                                 !(zBuddy_IsAvailable() &&
...
static void FodderTough_WoundWithKnock(zNPCRobot* npc, xPsyche* psy, NPCDamageInfo* info);

S32 zNPCGoalAlertFodder::NPCMessage(NPCMsg* mail)
{
    S32 snarfed = 1;
    zNPCRobot* npc = (zNPCRobot*)psyche->clt_owner;
    ...
    {
    case NPC_MID_DAMAGE:
        // Normal Fodder has no WOUND goal. Its damage messages must continue
        // through the stock RoboHandleMail()/zNPCGoalDamage path.
        if (npc->SelfType() != NPC_TYPE_FODDERTOUGH)
        {
            snarfed = 0;
            break;
        }

        if ((psy->GIDInStack(NPC_GOAL_WOUND) != NULL) ||
            (psy->GIDOfPending() == NPC_GOAL_WOUND))
        {
            break;
        }

        // This is the same architecture used by the multihit robot goals:
        // consume SIDE/HITBYTOSS while more than one HP remains and push WOUND.
        // The final HP and all other damage types fall through to normal death.
        if ((npc->hitpoints > 1) && (mail->infotype == NPC_MDAT_DAMAGE))
        {
            ...
            {
                flg_info |= 2;
                FodderTough_WoundWithKnock(npc, psy, &mail->dmgdata);
                break;
            }
        }
...
S32 zNPCGoalAlertFodder::CheckSpot(F32 dt)
{
    ...
    zBuddy_GetPreferredTarget(npc->Pos(), &pos_plyr);
...
void zNPCGoalAlertFodder::FlankPlayer(F32 dt)
{
    ...
    zBuddy_GetPreferredTarget(npc->Pos(), &pos_plyr);
    xVec3Sub(&dir_plyr, &pos_plyr, npc->Pos());
...
void zNPCGoalAlertFodBzzt::OrbitPlayer(F32 dt)
{
    ...
    zBuddy_GetPreferredTarget(npc->Pos(), &pos_plyr);
    npc->XZVecToPos(&dir_plyr, &pos_plyr, NULL);
    ...
    {
        xVec3Copy(&dir_plyr, NPCC_faceDir(npc));
    }
...
void zNPCGoalAlertFodBzzt::DeathRayUpdate(F32 dt)
{
    ...
    {
        pos_tgt = *(const xVec3*)NPCC_faceDir(npc) * dst_range;
        pos_tgt += *npc->Pos();
    }

    xVec3 dir_laser = pos_tgt - pos_src;
    dir_laser.normalize();
    xVec3 delta_target = pos_tgt - pos_src;
    F32 dst_target = xVec3Length(&delta_target);
    ...
    if (cnt_nextlos < 0)
    {
        ...
        if (buddy_target)
        {
            if (dst_target <= len_laser + 0.25f)
            {
                len_laser = dst_target;
                cnt_inContact++;
        ...
        else if (NPCC_LineHitsBound(&pos_src, &pos_tgt, &globals.player.ent.bound, colrec))
        {
...
void zNPCGoalAlertChomper::CirclePlayer(F32 dt)
{
    ...
    if (zBuddy_IsAvailable() && npc->arena.IncludesPos((xVec3*)zBuddy_GetPosition(), 0.0f, NULL) &&
        zBuddy_IsCloserTarget(npc->Pos()))
    {
        pos_plyr = *zBuddy_GetPosition();
    }
    ...
    if (length < 1.0f)
    {
        xVec3Copy(&dir_plyr, NPCC_rightDir(npc));
    }
    else
    {
        xVec3SMulBy(&dir_plyr, 1.0f / length);
...
S32 zNPCGoalAlertChomper::CheckSpot(F32 dt)
{
    ...
    else if (npc->XZDstSqToPlayer(NULL, NULL) < npc->XZDstSqToPos(&pos_plyr, NULL, NULL))
    {
...
S32 zNPCGoalAlertArf::Process(en_trantype* trantype, F32 dt, void* updCtxt, xScene* xscn)
{
    ...
    S32 buddy_in_arena = zBuddy_IsAvailable() &&
                         npc->arena.IncludesPos((xVec3*)zBuddy_GetPosition(), 0.0f, NULL);
    ...
    else if (globals.player.DamageTimer > 0.5f && !buddy_in_arena)
    {
        taunt = (zNPCGoalTaunt*)(psyche->FindGoal(NPC_GOAL_TAUNT));
        taunt->LoopCountSet(1);
        *trantype = GOAL_TRAN_PUSH;
    ...
    else if (dsq > SQ(rad) && !buddy_in_arena)
    {
        *trantype = GOAL_TRAN_SET;
        nextgoal = NPC_GOAL_IDLE;
    }
...
en_arfdoes zNPCGoalAlertArf::DecideAttack()
{
    ...
    F32 target_dsq = npc->XYZDstSqToPlayer(NULL);
    if (zBuddy_IsAvailable())
    {
        target_dsq = MIN(target_dsq, xVec3Dist2(npc->Pos(), zBuddy_GetPosition()));
    }

    if (target_dsq < SQ(3.0f))
    {
    ...
    if (this->haz_cattle)
    {
        if (this->haz_cattle->ConfigHelper(NPC_HAZ_CATTLEPROD))
        {
            ...
            if (zBuddy_IsAvailable() && npc->arena.IncludesPos((xVec3*)zBuddy_GetPosition(), 0.0f, NULL) &&
                zBuddy_IsCloserTarget(npc->Pos()))
            {
                this->haz_cattle->SetBuddyTarget(zBuddy_GetPosition());
            }
...
static void FodderTough_WoundWithKnock(zNPCRobot* npc, xPsyche* psy, NPCDamageInfo* info)
{
    zNPCGoalKnock* knock = (zNPCGoalKnock*)psy->FindGoal(NPC_GOAL_KNOCK);

    // Reuse the real knock goal's hit-origin calculation. We deliberately do
    // not enter NPC_GOAL_KNOCK itself because that goal also owns the normal
    // one-hit/death transition.
    if (knock != NULL)
    {
        knock->InputInfo(info);
    }

    psy->GoalSwap(NPC_GOAL_WOUND, 0);
    ...
    if (knock != NULL)
    {
        ...
        if (xVec3Length2(&knock->pos_bumper) > 0.001f)
        {
            npc->XZVecToPos(&dir_aim, &knock->pos_bumper, NULL);
        }
        else
        {
...
S32 zNPCGoalAttackFodder::NPCMessage(NPCMsg* mail)
{
    S32 snarfed = 1;
    zNPCRobot* npc = (zNPCRobot*)psyche->clt_owner;
    xPsyche* psy = (xPsyche*)xGoal::GetPsyche();

    switch (mail->msgid)
    {
    case NPC_MID_DAMAGE:
        // During combat the attack goal is often the current goal. Without
        // this interception, a player SIDE/HITBYTOSS message falls through
        // to the generic damage goal and kills Tough Fodder on hit #1.
        if ((npc->SelfType() == NPC_TYPE_FODDERTOUGH) &&
            (npc->hitpoints > 1) &&
...
S32 zNPCGoalAttackHammer::PlayerTests(xVec3* pos_vert, F32 dt)
{
    ...
    if (zBuddy_IsAvailable())
    {
        zBuddy_HitByRobot(&bnd.sph.center, bnd.sph.r);
    }
...
S32 zNPCGoalAttackHammer::ShockwaveTests(xVec3* pos_vert, F32 dt)
{
    ...
    if (zBuddy_IsAvailable())
    {
        zBuddy_HitByRobot(&bnd.sph.center, 2.25f);
    }
...
{
    static en_NPCTYPES totypes[5] = { NPC_TYPE_FODDER, NPC_TYPE_FODDERTOUGH, NPC_TYPE_FODBOMB, NPC_TYPE_CHOMPER,
                                      NPC_TYPE_UNKNOWN };
...
S32 zNPCGoalAttackTarTar::ShootBlob(F32, S32 zapidx)
{
    ...
    if (zBuddy_IsAvailable())
    {
        ...
        if (zBuddy_IsCloserTarget(npc->Pos()))
        {
            pos_tgt = *zBuddy_GetPosition();
            if (dst_miss != 0.0f)
            {
                xVec3AddScaled(&pos_tgt, NPCC_rightDir(npc), dst_miss);
            }
            xVec3 delta_target;
            xVec3Sub(&delta_target, &pos_tgt, &pos_launch);
            dist = xVec3Length(&delta_target);
        }
    }
...
S32 zNPCGoalAttackMonsoon::SpitCloud(F32 dt)
{
    ...
    haz->SetNPCOwner(npc);
    NPCArena* arena = &npc->arena;
    if (zBuddy_IsAvailable() && arena->IncludesPos((xVec3*)zBuddy_GetPosition(), 0.0f, NULL) &&
        zBuddy_IsCloserTarget(npc->Pos()))
    {
        haz->SetBuddyTarget(zBuddy_GetPosition());
    }
...
void zNPCGoalAttackArfMelee::PlayerTests()
{
    ...
    for (S32 i = 0; i < 2; i++)
    {
        ...
        if (zBuddy_IsAvailable())
        {
            zBuddy_HitByRobot(&bnd.sph.center, bnd.sph.r + 0.6f);
        }

        if (!(globals.player.DamageTimer > 0.0f) && NPCC_chk_hitPlyr(&bnd, NULL))
        {
            if (zEntPlayer_DamageNPCKnockBack(npc, 1, npc->Pos()) != 0)
            {
                npc->Vibrate(NPC_VIBE_HARD, -1.0f);
    ...
    if (zBuddy_IsAvailable())
    {
        xVec3 center = *npc->Pos();
        xVec3AddScaled(&center, NPCC_faceDir(npc), 1.0f);
        center.y += 0.5f;
        zBuddy_HitByRobot(&center, 1.25f);
    }
}
...
S32 zNPCGoalAttackArf::LaunchDoggie(F32 dt)
{
    ...
    if (zBuddy_IsAvailable() && npc->arena.IncludesPos((xVec3*)zBuddy_GetPosition(), 0.0f, NULL) &&
        zBuddy_IsCloserTarget(npc->Pos()))
    {
        pos_land = *zBuddy_GetPosition();
        pos_land.y = npc->Pos()->y;
    }
...
{
    zNPCSlick* npc = ((zNPCSlick*)(psyche->clt_owner));
    S32 rc = npc->LaunchProjectile(NPC_HAZ_OILBUBBLE, 9.0f, 4.0f, NPC_MDLVERT_ATTACK, 4.0f, 0.1f);

    // No-op on plain zNPCSlick; zNPCSlickBurst overrides this to add a
    // radial spread of extra shots on top of the normal aimed one.
    npc->FireRing(7.0f);
...
S32 zNPCGoalWound::Process(en_trantype* trantype, F32 dt, void* updCtxt, xScene* xscn)
{
    ...
    if (*trantype != GOAL_TRAN_NONE)
    {
        // Stock NPC_GOAL_KNOCK calls VelStop() when it exits. Tough Fodder
        // deliberately does not enter that goal, so reproduce its stop here
        // when the WOUND sequence finishes; otherwise the launch velocity can
        // survive into the next goal and make Fodder slide backwards.
        if (npc->SelfType() == NPC_TYPE_FODDERTOUGH)
        {
            npc->VelStop();
        }
    ...
    // PushAnim::Process can also request the transition directly (for example
    // when the wound animation finishes), bypassing the check above. Match the
    // stock knock goal's Exit() behavior in that case as well.
    if ((npc->SelfType() == NPC_TYPE_FODDERTOUGH) &&
        (*trantype != GOAL_TRAN_NONE))
    {
        npc->VelStop();
    }
...
S32 zNPCGoalRespawn::Exit(F32 dt, void* updCtxt)
{
    ...
    KickFromTheNest();
    xEntShow(npc);
    if (npc->SelfType() == NPC_TYPE_ARFDOG)
    {
        zBuddy_ForgetTarget((zNPCCommon*)npc);
    }
    zNPCCommon* duper = npc->npc_duplodude;
    if (duper != NULL)
    {
...
void zNPCGoalTubeAttack::MaryzFury()
{
    zNPCTubelet* npc = (zNPCTubelet*)psyche->clt_owner;
    NPCHazard* haz = HAZ_Acquire();
    if (haz != NULL && npc != NULL && haz->ConfigHelper(NPC_HAZ_TUBELETBLAST))
    {
        haz->SetNPCOwner(npc);
        if (zBuddy_IsAvailable() && npc->arena.IncludesPos((xVec3*)zBuddy_GetPosition(), 0.0f, NULL) &&
            zBuddy_IsCloserTarget(npc->Pos()))
...
static RoboCopMap g_map_policeCounter[18] = {
    // clang-format off
    ...
    { NPC_TYPE_FODDERTOUGH, ROBOCOP_CNTR_FODDER },
