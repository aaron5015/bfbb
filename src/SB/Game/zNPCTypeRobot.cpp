...
xFactoryInst* ZNPC_Create_Robot(S32 who, RyzMemGrow* grow, void*)
{
    ...
    {
        robo = new (who, grow) zNPCFodderTough(who);
        break;
    }
    ...
    {
        robo = new (who, grow) zNPCSlickTouhou(who);
        break;
    }
    default:
    {
...
void zNPCRobot::Process(xScene* xscn, F32 dt)
{
    ...
    if (IsAlive())
    {
        S32 goal = psy_instinct->GIDOfActive();
        // Buddy targeting is not routed through the player-only attack goals yet.
    }
...
void zNPCFodder::SelfSetup()
{
    ...
    // Only Tough Fodder gets the reusable wound/recovery goal. Normal Fodder
    // intentionally keeps its vanilla one-hit behavior.
    if (SelfType() == NPC_TYPE_FODDERTOUGH)
    {
...
void zNPCFodderTough::Stun(F32 stuntime)
{
    // Vanilla Fodder refuses subsequent Stun messages after becoming wounded.
    // Tough Fodder instead lets each player hit enter the normal Damage() path;
    // its AlertFodder goal decides whether that damage is a wound or a death.
    xVec3 dir_dmg = { 0.0f, 0.0f, 0.0f };
...
void zNPCArfArf::DuploNotice(en_SM_NOTICES note, void* data)
{
    ...
    case SM_NOTE_NPCDIED:
        zBuddy_ForgetTarget((zNPCCommon*)data);
        for (i = 0; i < 5; i++)
        {
...
// Touhou-esque flourish for the Slick fork: on top of Slick's normal aimed
// shot (still fired by zNPCGoalAttackSlick::FireOne via LaunchProjectile),
// spin out a full ring of oil bubbles around the NPC. Each bubble is aimed
// at a fixed point on a circle rather than at the player, so unlike
// LaunchProjectile() this doesn't touch player-prediction at all - it just
// reuses the same "set an explicit pos_tgt and Start()" pattern that
// zNPCGoalAttackTarTar::ShootBlob uses for its own manual-aim shots.
S32 zNPCSlickTouhou::FireRing(F32 spd_proj)
{
    static const S32 cnt_ring = 16;

    xVec3 pos_launch;

    if (!GetVertPos(NPC_MDLVERT_ATTACK, &pos_launch))
    {
    ...
    F32 dst_plyr = xsqrt(XZDstSqToPlayer(NULL, NULL));
    F32 rad_ring = CLAMP(0.5f * dst_plyr, 1.0f, 9.0f);

    S32 cnt_fired = 0;

    for (S32 i = 0; i < cnt_ring; i++)
    {
        ...
        if (!haz->ConfigHelper(NPC_HAZ_OILBUBBLE))
        {
            haz->Discard();
            continue;
        }
        ...
        F32 ang_seg = ((2.0f * PI) / cnt_ring) * i;

        xVec3 dir_ring = { isin(ang_seg), 0.0f, icos(ang_seg) };
        xVec3 dir_tangent = { dir_ring.z, 0.0f, -dir_ring.x };
        xVec3SMulBy(&dir_tangent, 0.2f);
        xVec3AddTo(&dir_ring, &dir_tangent);
        xVec3Normalize(&dir_ring, &dir_ring);
        xVec3 pos_tgt;

        xVec3SMul(&pos_tgt, &dir_ring, rad_ring);
        xVec3AddTo(&pos_tgt, &pos_launch);
        pos_tgt.y -= 1.0f;

        xVec3Copy(&haz->custdata.tartar.pos_tgt, &pos_tgt);
        xVec3 ang_spin = { 0.0f, 0.0f, (i & 1) ? -0.016666668f : 0.016666668f };
        haz->TypData_RotMatStore(&ang_spin);
        haz->Start(&pos_launch, MAX(0.25f, rad_ring / MAX(spd_proj, 0.25f)));
